...
def home():
    ...
    if (hour >= 0 and hour <= 5) or (hour >= 18 and hour <= 24):
        return render_template('perday.html', currTimeprediction=0, solarOutputPerhours=solarOutputPerhours, time=times, solarOutputPerDay=solarOutputPerDay, costsavings=costsavings, averageSolarEnergyPerHour=averageSolarEnergyPerHour, co2=co2, city_name=city_name, lat=lat, long=long, endDate=endDate, co2NoOfTree=int(co2/21))
    X = list([X])

    pred = round(model.predict(X)[0], 3)
    return render_template('perday.html', currTimeprediction=pred, solarOutputPerhours=solarOutputPerhours, time=times, solarOutputPerDay=solarOutputPerDay, costsavings=costsavings, averageSolarEnergyPerHour=averageSolarEnergyPerHour, co2=round(co2, 2), city_name=city_name, lat=lat, long=long, endDate=endDate, co2NoOfTree=int(co2/21))
